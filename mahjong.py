...
function checkYaku(handNames, winNorm) {{
  const counts=handToCounts(handNames);
  if (isKokushi(counts)) {{
    const r=['國士無雙 (役滿)'];
  ...
  // ── 兩條路線都成立時，選翻數較高的 ──
  if (chitoiYaku && normalYaku) {{
    return sumHan(normalYaku) >= sumHan(chitoiYaku) ? normalYaku : chitoiYaku;
  }}
  return chitoiYaku || normalYaku || [];
}}
